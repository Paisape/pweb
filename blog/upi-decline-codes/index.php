...
<article class="bg-white pt-32 pb-16 md:pt-40 md:pb-24">
  <div class="mx-auto max-w-[800px] px-5">
    ...
    <div class="prose prose-lg prose-slate mt-10 max-w-none prose-headings:font-display prose-headings:font-bold prose-headings:tracking-tight prose-a:text-brand prose-a:font-semibold hover:prose-a:text-brandDk prose-h2:text-[24px] prose-h2:mt-12 prose-h2:mb-4 prose-p:text-[16px] prose-p:leading-loose prose-p:text-body prose-li:text-[16px]">
      <h2>Not All Failures Are Equal</h2><p>When a UPI transaction fails, the PG usually returns a generic 'FAILED' status. However, beneath the surface lies an NPCI response code. For example, <code>U19</code> means 'Transaction amount exceeds limit'. <code>U16</code> means 'Risk threshold exceeded'.</p><h2>Which Codes Should You Retry?</h2><p>A code like <code>U30</code> (Debit has been failed) or <code>91</code> (Issuer switch timeout) suggests a temporary banking hiccup. These are prime candidates for background retries or prompting the user to try a different bank account. Conversely, if you receive <code>U17</code> (Invalid PIN), no amount of technical retrying will save the transaction.</p><h2>Improving UX</h2><p>Instead of showing a generic 'Payment Failed' screen, use these codes to show actionable errors: 'You entered the wrong UPI PIN' or 'Your bank servers are currently unresponsive, please use a different account.'</p>
    </div>
  </div>
