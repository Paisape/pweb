...
<article class="bg-white pt-32 pb-16 md:pt-40 md:pb-24">
  <div class="mx-auto max-w-[800px] px-5">
    ...
    <div class="prose prose-lg prose-slate mt-10 max-w-none prose-headings:font-display prose-headings:font-bold prose-headings:tracking-tight prose-a:text-brand prose-a:font-semibold hover:prose-a:text-brandDk prose-h2:text-[24px] prose-h2:mt-12 prose-h2:mb-4 prose-p:text-[16px] prose-p:leading-loose prose-p:text-body prose-li:text-[16px]">
      <h2>The Backbone of Financial Inclusion</h2><p>AEPS allows online interoperable financial transactions at PoS (Point of Sale / Micro ATM) through the Business Correspondent (BC) of any bank using the Aadhaar authentication. The system handles massive volumes daily, especially in Tier 3 cities and beyond.</p><h2>How the Transaction Flows</h2><p>When a customer places their thumb on a biometric scanner: 1. The device captures the biometric and encrypts it using a PID block. 2. The BC application sends this to the Acquirer Bank. 3. The Acquirer forwards it to NPCI via the National Financial Switch. 4. NPCI routes it to UIDAI for authentication and then to the Issuer Bank to check the balance and debit the account.</p><h2>Common Failure Points</h2><p>Biometric mismatch and poor connectivity are the two highest causes of transaction failure. Ensuring robust offline capabilities and clear failure messaging is critical for BC retention.</p>
    </div>
  </div>
