...
<article class="bg-white pt-32 pb-16 md:pt-40 md:pb-24">
  <div class="mx-auto max-w-[800px] px-5">
    ...
    <div class="prose prose-lg prose-slate mt-10 max-w-none prose-headings:font-display prose-headings:font-bold prose-headings:tracking-tight prose-a:text-brand prose-a:font-semibold hover:prose-a:text-brandDk prose-h2:text-[24px] prose-h2:mt-12 prose-h2:mb-4 prose-p:text-[16px] prose-p:leading-loose prose-p:text-body prose-li:text-[16px]">
      <h2>The Danger of Blind Retries</h2><p>If a transaction times out, your system might assume it failed and attempt to route the payment through a secondary gateway. The next day, the customer checks their bank statement and sees two debits. You've just created a chargeback nightmare.</p><h2>Idempotency Keys</h2><p>Always use a unique transaction ID generated by your system (the Idempotency Key) and pass it to the PG. If a network timeout occurs, query the PG using this exact key to get the definitive status before ever attempting a retry.</p><h2>The 'Pending' Purgatory</h2><p>Sometimes, a transaction remains 'Pending' for hours. Your application logic must strictly forbid a retry on the same order if a previous attempt is in a pending state. Instruct the customer to wait, as banks will auto-reverse failed pending transactions within 48 hours.</p>
    </div>
  </div>
