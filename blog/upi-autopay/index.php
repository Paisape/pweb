...
<article class="bg-white pt-32 pb-16 md:pt-40 md:pb-24">
  <div class="mx-auto max-w-[800px] px-5">
    ...
    <div class="prose prose-lg prose-slate mt-10 max-w-none prose-headings:font-display prose-headings:font-bold prose-headings:tracking-tight prose-a:text-brand prose-a:font-semibold hover:prose-a:text-brandDk prose-h2:text-[24px] prose-h2:mt-12 prose-h2:mb-4 prose-p:text-[16px] prose-p:leading-loose prose-p:text-body prose-li:text-[16px]">
      <h2>The End of eNACH?</h2><p>Before UPI Autopay, recurring payments in India required eNACH mandates, which took days to clear and suffered from high drop-off rates. UPI Autopay allows instant mandate creation using a simple UPI PIN.</p><h2>The Pre-Debit Notification (PN) Rule</h2><p>As per RBI guidelines, you cannot simply debit a customer's account blindly. You MUST send a Pre-Debit Notification exactly 24 hours before the actual debit execution. If you miss this window, the debit will be universally declined by the issuer bank.</p><h2>Managing Mandate Pauses and Revocations</h2><p>Customers have full control over their mandates via their UPI app (PhonePe, GPay). Your webhook infrastructure must be robust enough to handle unexpected <code>MANDATE_REVOKED</code> callbacks, instantly updating the user's subscription status in your database to avoid delivering free services.</p>
    </div>
  </div>
