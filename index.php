  ...
  <div class="marquee">
    <div class="track animate-marquee-slow gap-12 pr-12">
      <div class="flex shrink-0 items-center gap-12 pr-12" data-mq-group>
        ...
        <span class="whitespace-nowrap font-display text-[17px] font-extrabold text-brand transition hover:text-brandDk inline-flex items-center gap-1.5"><svg class="h-4 w-4 text-emerald-500" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="m9 12 2 2 4-4"/></svg>PayU Partner</span>
...
<section id="latest-blogs" class="bg-white border-t border-slate-100 pt-12 pb-16">
  <div class="mx-auto max-w-site px-5">
    ...
    <div class="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
      <?php
        require_once $_SERVER['DOCUMENT_ROOT'] . '/includes/blogs_data.php';
        $latest_blogs = get_latest_blogs(3);
        foreach ($latest_blogs as $blog):
      ?>
      <article class="card-lift flex flex-col overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
        <a href="<?php echo htmlspecialchars($blog['url']); ?>" class="block overflow-hidden h-44 bg-slate-100">
          <img src="<?php echo htmlspecialchars($blog['image']); ?>" alt="<?php echo htmlspecialchars($blog['title']); ?>" class="h-full w-full object-cover transition duration-500 hover:scale-105" />
        </a>
        <div class="flex flex-1 flex-col p-6">
          <div class="flex items-center gap-3 text-[11px] font-bold uppercase tracking-[0.14em]">
            <span class="text-brand"><?php echo htmlspecialchars($blog['category']); ?></span>
            <span class="text-body/50"><?php echo htmlspecialchars($blog['read_time']); ?></span>
          </div>
          <a href="<?php echo htmlspecialchars($blog['url']); ?>">
            <h3 class="mt-3 font-display text-[15px] font-bold leading-snug text-ink hover:text-brand"><?php echo htmlspecialchars($blog['title']); ?></h3>
          </a>
          <p class="mt-2 text-[12.5px] leading-relaxed text-body line-clamp-3"><?php echo htmlspecialchars($blog['description']); ?></p>
          <div class="mt-5 flex items-center justify-between border-t border-slate-100 pt-4">
            <span class="text-[11px] text-body/70"><?php echo htmlspecialchars($blog['date']); ?></span>
            <a href="<?php echo htmlspecialchars($blog['url']); ?>" class="inline-flex items-center gap-1.5 text-[12px] font-semibold text-brand">Read <span class="arrow">&rarr;</span></a>
          </div>
        </div>
      </article>
      <?php endforeach; ?>
